    ...
    <tbody class="divide-y divide-gray-200 dark:divide-secondary-light">
      <tr v-for="(role_id, index) in this.$props.selectedRoles" :key="index">
        <td class="pr-3 text-sm dark:text-gray-50 py-3.5 text-left w-4/5">
          <font-awesome-icon
            icon="circle"
            class="text-gray-400 inline w-4 h-4 mr-1 border-primary"
            :style="$store.getters.getGuildRoleById(role_id)?.color ? {
              color: `${getHexColor(
        ...
        <td
          class="whitespace-nowrap py-4 text-sm text-center dark:text-gray-50 space-x-2 py-3.5"
        >
          <a
            @click="this.onRemoveRole(role_id)"
              ...
              >
                ...
                >
                  ...
                  <div v-else>
                    <ListboxOption
                      as="template"
                      v-for="role in this.selectableRoles"
                      :key="role.id"
                      :value="role.id"
