  ...
  <div>
    ...
    >
      <div class="contents-2mQqc9" role="document">
        ...
        <div class="markup-2BOw-j messageContent-2qWWxC" v-html="marked(content, true)" />
      </div>
      <div class="container-1ov-mD" v-for="embed in embeds" v-bind:key="embed">
        <!-- <pre class="p-2 text-gray-200 bg-gray-700 rounded-lg">{{embed}}</pre> -->
        <div
          class=" embedWrapper-lXpS3L embedFull-2tM8-- embed-IeVjo6 markup-2BOw-j"
          aria-hidden="false"
          style="border-color: #2563eb"
        >
          <div :class="['grid-1nZz7S', embed?.thumbnail?.url != '' ? 'hasThumbnail-3FJf1w' : '']">
            ...
            <div class="embedDescription-1Cuq9a embedMargin-UO5XwE" v-html="marked(embed?.description, true)" v-if="embed?.description" />
            <div class="embedFields-2IPs5Z">
              <div class="embedField-1v-Pnh" :style="'grid-column: ' + (field.inline ? (field.odd ? '7 / 13' : '1 / 7') : '1 / 13')" v-for="field in embed?.fields" v-bind:key="field">
                <div class="embedFieldName-NFrena">
                  <span class="emojiContainer-3X8SvE" role="button" tabindex="0" v-html="marked(field.name, true)" />
                </div>
                <div class="embedFieldValue-nELq2s"><span v-html="marked(field.value, true)" /></div>
              </div>
            </div>
            <a
            ...
            <div class="embedFooter-3yVop- embedMargin-UO5XwE">
              <span class="embedFooterText-28V_Wb"
                >{{embed?.footer}}<span class="embedFooterSeparator-3klTIQ" v-if="embed?.footer">•</span
                >Today at 17:26</span
              >
            </div>
          </div>
